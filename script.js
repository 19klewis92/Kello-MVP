...
const TRANSLATIONS = {
  ko: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: '사용하실 언어를 선택해주세요\n(Select Language)', options: ['영어', '일본어', '중국어'], autoNext: true },
      { id: 'q1', type: 'single', label: 'Step 1', title: '한국에 방문한 적 있거나\n방문 계획이 있나요?', options: ['예', '아니오'], autoNext: true },
      { id: 'q2', type: 'single', label: 'Step 2', title: '한국에서 예약 서비스를\n이용해본 적이 있나요?\n<span class="small-title">(식당, 뷰티샵, 호텔 등)</span>', options: ['있음', '없음'], autoNext: true },
      { id: 'q3', type: 'multiple', maxSelect: 3, label: 'Step 3', title: '한국에서 해보고 싶은 서비스는?', options: ['헤어', '메이크업', '네일', '피부관리', '왁싱', '속눈썹', '타투', '두피관리', '세신', '기타'], hasEtc: true, grid: true },
      { id: 'q4', type: 'multiple', maxSelect: 3, label: 'Step 4', title: '뷰티샵 예약 시 불편한 점은?', options: ['언어 문제', '한국번호 인증', '가격 불신', '업체 정보 부족', '예약 시간 문제', '기타'], hasEtc: true },
      { id: 'q5', type: 'multiple', maxSelect: 3, label: 'Step 5', title: '필요한 서비스 기능은?', options: ['자동 번역 예약', '예약 대행', '신뢰 가능한 업체/가격', '네비게이션', '대중교통 안내', '택시 호출', '커뮤니티 정보 공유', '기타'], hasEtc: true },
      { id: 'q6', type: 'single', label: 'Step 6', title: '이 문제가 해결된다면,\n예약 서비스에 대한 이용료를 낼 의향이 있나요?', options: ['지불 의사 있음', '조건에 따라 가능', '지불 의사 없음'], autoNext: true },
    ]
  },
  en: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: 'Please select your language', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
  ...
  ja: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: '言語を選択してください', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
  ...
  zh: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: '请选择您的语言', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
  ...
  vi: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: 'Vui lòng chọn ngôn ngữ của bạn', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
  ...
  th: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: 'กรุณาเลือกภาษาของคุณ', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
  ...
  mn: {
    ...
    steps: [
      { id: 'q0', type: 'single', label: 'Start', title: 'Хэлээ сонгоно уу', options: ['English', 'Japanese', 'Chinese'], autoNext: true },
...
async function initLanguage() {
  ...
  try {
    ...
    const countryToLang = { 'KR': 'ko', 'JP': 'ja', 'CN': 'zh', 'VN': 'vi', 'TH': 'th', 'MN': 'mn' };
    const lang = countryToLang[geo.country_code] || 'en';
    setLanguage(lang);
  } catch (e) {
    setLanguage('en');
  }
...
// ── Logic ──────────────────────────────────────────────────────
function toKorean(stepId, value) {
  const koStep = TRANSLATIONS.ko.steps.find(s => s.id === stepId);
  const currentStepData = TRANSLATIONS[currentLang].steps.find(s => s.id === stepId);
  if (!koStep || !currentStepData) return value;

  if (Array.isArray(value)) {
    return value.map(val => {
      const idx = currentStepData.options.indexOf(val);
      return idx > -1 ? koStep.options[idx] : val;
    });
  } else {
    const idx = currentStepData.options.indexOf(value);
    return idx > -1 ? koStep.options[idx] : value;
  }
}
...
function render(idx) {
  ...
  container.innerHTML = `
    ...
    <h2 class="question-title">${step.title.replace(/\n/g, '<br>')}</h2>
    ${step.type === 'multiple' ? `<p class="max-select-msg">${t.maxSelectMsg}</p>` : ''}
    <div class="options-container ${step.grid ? 'grid' : ''}"></div>
    ${step.hasEtc ? `
      <div class="etc-container" id="etc-box-${step.id}">
        <div class="etc-input-group">
          <input type="text" id="etc-input-${step.id}" placeholder="${t.etcPlaceholder}">
          <button class="etc-add-btn" id="etc-add-${step.id}">+</button>
        </div>
        <div class="etc-tags" id="etc-tags-${step.id}"></div>
      </div>
    ` : ''}
    <div class="nav-btns">
      ${idx > 0 ? `<button class="nav-btn prev" id="prev-btn">${t.prev}</button>` : '<div></div>'}
      <button class="nav-btn next" id="next-btn">${idx === t.steps.length - 1 ? t.finish : t.next}</button>
    </div>
  `;
  ...
  step.options.forEach(opt => {
    ...
    btn.textContent = opt;
    if (step.type === 'single' && answers[step.id] === opt) btn.classList.add('selected');
    if (step.type === 'multiple' && answers[step.id]?.includes(opt)) btn.classList.add('selected');
    optionsBox.appendChild(btn);
  });

  if (step.hasEtc) renderEtcTags(step.id);
  attachStepListeners(step);
}

function renderEtcTags(stepId) {
  const tagBox = document.getElementById(`etc-tags-${stepId}`);
  if (!tagBox) return;
  tagBox.innerHTML = '';
  etcTags[stepId]?.forEach((tag, i) => {
    const span = document.createElement('span');
    span.className = 'etc-tag';
    span.innerHTML = `${tag} <i class="close-tag">&times;</i>`;
    span.querySelector('.close-tag').onclick = () => {
      etcTags[stepId].splice(i, 1);
      renderEtcTags(stepId);
    };
    tagBox.appendChild(span);
  });
}

function addTag(stepId) {
  const input = document.getElementById(`etc-input-${stepId}`);
  const val = input.value.trim();
  if (val && !etcTags[stepId].includes(val)) {
    if (etcTags[stepId].length >= 3) return;
...
function attachStepListeners(step) {
  ...
  const etcBox = document.getElementById(`etc-box-${step.id}`);
  const etcAddBtn = document.getElementById(`etc-add-${step.id}`);
  const etcInput = document.getElementById(`etc-input-${step.id}`);
  ...
  container.querySelectorAll('.option-btn').forEach(btn => {
    btn.onclick = () => {
      ...
      if (step.type === 'single') {
        ...
        // q0 Special Logic: Language selection
        if (step.id === 'q0') {
          const langMap = { 0: 'en', 1: 'ja', 2: 'zh' };
          const idx = step.options.indexOf(value);
          if (langMap[idx] !== undefined) {
            currentLang = langMap[idx];
            document.querySelectorAll('.lang-tab').forEach(t => {
              if (t.dataset.lang === currentLang) t.classList.add('active');
              else t.classList.remove('active');
            });
          }
        }
        ...
        if (idx > -1) {
          ...
          if (step.maxSelect && answers[step.id].length >= step.maxSelect) return;
  ...
  if (etcAddBtn) etcAddBtn.onclick = () => addTag(step.id);
  if (etcInput) etcInput.onkeypress = (e) => { if (e.key === 'Enter') addTag(step.id); };
  
  if (nextBtn) {
    nextBtn.onclick = () => {
      const ans = answers[step.id];
      const hasEtcTags = etcTags[step.id]?.length > 0;
      if ((!ans || (Array.isArray(ans) && ans.length === 0)) && !hasEtcTags) {
...
async function finish() {
  ...
  const required = ['q0', 'q1', 'q2', 'q3', 'q4', 'q5', 'q6'];
  for (const rid of required) {
    const ans = answers[rid];
    const hasEtcTags = etcTags[rid]?.length > 0;
    if ((!ans || (Array.isArray(ans) && ans.length === 0)) && !hasEtcTags) {
      alert('Some questions are not answered. Please complete all steps.');
      return;
    }
  }
  ...
  // 3. Map all data to Korean
  const data = {
    user_language:      toKorean('q0', answers['q0'] || ''),
    visit_korea:        toKorean('q1', answers['q1'] || ''),
    booking_experience: toKorean('q2', answers['q2'] || ''),
    desired_services:   [...toKorean('q3', answers['q3'] || []), ...(etcTags.q3 || [])],
    pain_points:        [...toKorean('q4', answers['q4'] || []), ...(etcTags.q4 || [])],
    needed_features:    [...toKorean('q5', answers['q5'] || []), ...(etcTags.q5 || [])],
  ...
  // 5. Save to survey_responses_v3
  await saveSurvey(data);

  // 6. Redirect to official site
  setTimeout(() => {
